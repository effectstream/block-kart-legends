...
import { carVertexShader, carFragmentShader } from './shaders/proceduralMaterials.ts';
import { createItemModel } from './models/index.ts';
import { ActiveEffect } from './GameSimulator.ts';
...
export class Car {
    ...
    constructor(scene: THREE.Scene, circuit: Circuit, id: string, stats: VehicleStats, color: string | number, isPlayer: boolean = false) {
        this.scene = scene;
        ...
        const highlightColor = new THREE.Color().setHSL(hsl.h, Math.max(0, Math.min(1, hsl.s + 0.2)), Math.min(1, hsl.l + 0.3));
        ...
        cabin.position.y = chassis.position.y + chassisHeight/2 + cabinHeight/2;
        ...
        const windshieldGeometry = new THREE.BoxGeometry(cabinWidth - 0.02, cabinHeight - 0.02, 0.05);
        ...
        const rearWindowGeometry = new THREE.BoxGeometry(cabinWidth - 0.02, cabinHeight - 0.02, 0.05);
    ...
    public updateVisualEffects(activeEffects: ActiveEffect[]) {
        ...
        if (effect.type !== this.currentEffectType) {
            ...
            if (effect.type === 'Invincible') {
                 ...
                 const model = effect.label === 'Hit' ? createItemModel('Red Shell') : createItemModel('Banana');
