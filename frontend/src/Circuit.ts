...
import { trackVertexShader, trackFragmentShader } from './shaders/proceduralMaterials.ts';
...
export class Circuit {
    ...
    constructor(scene: THREE.Scene, points: THREE.Vector3[], config?: TrackConfig) {
        this.scene = scene;
        this.config = config || {
    ...
    private createTrackMaterial(): THREE.ShaderMaterial {
        let trackType = 0;
        if (this.config.condition === TrackCondition.DIRT) trackType = 1;
        else if (this.config.condition === TrackCondition.ICE) trackType = 2; // Treat ICE as Snow
    ...
    private createLineMarkings(offset: number) {
        ...
        if (this.config.condition === TrackCondition.ICE) {
        ...
        for (let i = 0; i < divisions; i++) {
            ...
            const side1 = new THREE.Vector3().crossVectors(tan1, up).normalize().multiplyScalar(offset);
            const center1 = p1.clone().add(side1);
            const perp1 = new THREE.Vector3().crossVectors(tan1, up).normalize().multiplyScalar(width / 2);

            // Point 2
            const p2 = this.curve.getPointAt(t2 % 1);
            const tan2 = this.curve.getTangentAt(t2 % 1).normalize();
            const side2 = new THREE.Vector3().crossVectors(tan2, up).normalize().multiplyScalar(offset);
            const center2 = p2.clone().add(side2);
            const perp2 = new THREE.Vector3().crossVectors(tan2, up).normalize().multiplyScalar(width / 2);
    ...
    private createStartArc() {
        ...
        const matrix = new THREE.Matrix4();
        matrix.makeBasis(side, up, tangent);
        group.setRotationFromQuaternion(new THREE.Quaternion().setFromRotationMatrix(matrix));

        // Pillars
        const pillarWidth = 0.3;
        const pillarGeo = new THREE.BoxGeometry(pillarWidth, height, pillarWidth);
    ...
    private createGrass() {
        ...
        const material = new THREE.MeshBasicMaterial({ color: this.config.grassColor }); 
    ...
    public getTrackMask(): { texture: THREE.Texture, bounds: { minX: number, maxX: number, minZ: number, maxZ: number } } {
