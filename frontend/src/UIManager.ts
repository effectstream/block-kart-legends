import { VehicleStats, PlayerConfig, ItemType, ItemPlacement } from './types.ts';
import { ItemThumbnailGenerator } from './ItemThumbnailGenerator.ts';
import { LeaderboardEntry } from './MockService.ts';
...
export class UIManager {
    ...
    constructor() {
        ...
        // Inputs
        this.speedInput = document.getElementById('stat-speed') as HTMLInputElement;
        this.accelInput = document.getElementById('stat-accel') as HTMLInputElement;
        this.weightInput = document.getElementById('stat-weight') as HTMLInputElement;
        this.handlingInput = document.getElementById('stat-handling') as HTMLInputElement;
        this.tractionInput = document.getElementById('stat-traction') as HTMLInputElement;
        ...
        this.itemsContainer = document.getElementById('available-items-container')!;
        this.slotsContainer = document.getElementById('slots-container')!;

        // Allow dropping back to pool
        this.itemsContainer.addEventListener('dragover', (e) => e.preventDefault());
        this.itemsContainer.addEventListener('drop', (e) => this.handleDropToPool(e));
        ...
        this.leaderboardList.addEventListener('mouseover', (e) => {
            ...
            if (target.classList.contains('leaderboard-item')) {
                 ...
                 if (data && data.stats) {
                      ...
                      this.showCarTooltip(rect.right, rect.top - 15, data.name, data.stats, null);
                 }
            }
        });
    ...
    private initListeners() {
        const inputs = [this.speedInput, this.accelInput, this.weightInput, this.handlingInput, this.tractionInput];
        const displays = [this.valSpeed, this.valAccel, this.valWeight, this.valHandling, this.valTraction];
    ...
    public showScreen(screenId: string) {
        ...
        if (screenId === 'setup-screen' || screenId === 'start-screen' || screenId === 'results-screen') {
            this.speedDisplay.style.display = 'none';
            this.positionDisplay.style.display = 'none';
            this.itemDisplay.style.display = 'none';
            this.effectsDisplay.style.display = 'none';
    ...
    private loadFromStorage() {
        try {
            ...
            // Restore Stats
            if (config.stats) {
                if (this.speedInput) this.speedInput.value = config.stats.max_velocity.toString();
                if (this.accelInput) this.accelInput.value = config.stats.accel_curve.toString();
                if (this.weightInput) this.weightInput.value = config.stats.mass.toString();
                if (this.handlingInput) this.handlingInput.value = config.stats.turn_radius.toString();
                if (this.tractionInput) this.tractionInput.value = config.stats.grip_factor.toString();
                
                // Update display values
                if (this.valSpeed) this.valSpeed.innerText = this.speedInput.value;
                if (this.valAccel) this.valAccel.innerText = this.accelInput.value;
                if (this.valWeight) this.valWeight.innerText = this.weightInput.value;
                if (this.valHandling) this.valHandling.innerText = this.handlingInput.value;
                if (this.valTraction) this.valTraction.innerText = this.tractionInput.value;
            }
            
            // Restore Items
            if (config.items && Array.isArray(config.items)) {
                config.items.forEach(placement => {
                    if (placement.slotIndex >= 0 && placement.slotIndex < this.slots.length) {
    ...
    private generateAllItems() {
        ...
        this.availableItems = types.map((type, i) => ({ id: `item-source-${i}`, type }));
    }
    ...
    private renderItems() {
        ...
        const labels = [
            "L1 Start", "L1 Half", "L1 End",
            "L2 Start", "L2 Half", "L2 End",
            "L3 Start", "L3 Half", "L3 End",
            "Finish"
        ];
    ...
    private createItemElement(type: ItemType, index: number, isInSlot: boolean): HTMLElement {
        const el = document.createElement('div');
    ...
    public showCarTooltip(x: number, y: number, name: string, stats: VehicleStats, item: ItemType | null) {
    ...
    public updateLeaderboard(cars: { name: string, rank: number, lap: number, progress: number, finished: boolean, finishTime: number, stats?: VehicleStats }[]) {
        ...
        const getContent = (car: typeof cars[0]) => {
            const status = car.finished 
                ? `<span style="color:#0f0">FINISHED (${(car.finishTime / 1000).toFixed(2)}s)</span>` 
                : `Lap ${car.lap}`;
    ...
    public showResults(cars: { name: string, rank: number, finishTime: number }[]) {
        const container = document.getElementById('results-content');
        if (!container) return;

        let html = '<table style="width:100%; text-align:left; margin-bottom: 20px;">';
        ...
        sorted.forEach(car => {
            html += `<tr>
                <td>#${car.rank}</td>
                <td>${car.name}</td>
                <td>${car.finishTime > 0 ? (car.finishTime / 1000).toFixed(2) + 's' : '-'}</td>
            </tr>`;
        });
        html += '</table>';
