...
import { initWalletUI, initializeLocalWallet, updateSetNameButtonLabel, getConnectedWallet } from './Wallet.ts';
...
const init = () => {
    ...
    if (container) {
        ...
        try {
            ...
            new GameManager(containerId);
        } catch (e: any) {
            console.error("Failed to start game:", e);
            container.innerHTML = `<p style="color:red">Error starting game: ${e.message}</p><pre>${e.stack}</pre>`;
        }
    } else {
