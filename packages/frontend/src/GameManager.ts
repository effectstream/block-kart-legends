...
export class GameManager {
    ...
    private createCars(cars: CarConfig[]) {
        ...
        for (let i = 0; i < NUMBER_OF_BOTS; i++) {
            ...
            // Bot Random Item
            const randomItem = botCars[i].initialItem;
    ...
    private handlePlayerInput(deltaTime: number) {
        ...
        const simCar = this.simulator.getCars().find((c) =>
            c.id === this.playerCar!.id
        );
        ...
        if (simCar && simCar.currentSpeed > 0) {
            ...
            this.playerCar.horizontalOffset = Math.max(
                -trackWidthLimit,
                Math.min(trackWidthLimit, this.playerCar.horizontalOffset),
            );
        }
    }
        ...
        if (playerSimCar) {
            ...
            if (this.currentConfig && this.currentConfig.items) {
                ...
                if (slotIndex !== this.lastSlotIndex) {
                    ...
                    const itemEntry = this.currentConfig.items.find((i) =>
