...
import { TrackCondition, TrackConfig } from "./types.ts";
import { RNG } from "./RNG.ts";

export class TrackUtils {
    static generateTrackConfig(rnd?: RNG): TrackConfig {
        const conditions = [
            TrackCondition.ASPHALT,
            TrackCondition.DIRT,
            TrackCondition.ICE,
        ];
        const rng = rnd || RNG.getInstance();
    ...
    static generateRandomTrackPoints(rnd?: RNG): THREE.Vector3[] {
