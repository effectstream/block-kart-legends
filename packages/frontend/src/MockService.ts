import { RaceResult } from "./simulation/GameSimulator.ts";
import { ItemPlacement, VehicleStats } from "./simulation/types.ts";
import { sendTransaction } from "@paimaexample/wallets";
import { EngineConfig } from "./effectstream/EffectStreamEngineConfig.ts";
import { getLocalWallet, initializeLocalWallet } from "./Wallet.ts";
import { showToast } from "./Toast.ts";
import { accountPayload_ as accountPayload } from "@paimaexample/wallets";
import { ENV } from "./effectstream/EffectStreamEngineConfig.ts";

// Real network service for the game – backed by the Paima HTTP API.
...
export class MockService {
  ...
  > {
    try {
      const response = await fetch(`${API_URL}/api/gamestate/${walletAddress}`);
      if (!response.ok) {
        // Return default
        return {
          round: 1,
  ...
  public async getUserProfile(
    ...
    try {
      ...
      console.error("[MockService] getUserProfile error", e);
      const fallbackName = walletAddress.slice(0, 6) + "..." +
        walletAddress.slice(-4);
      return {
        id: "0",
  ...
  public async getAddressInfo(
    ...
    try {
      const res = await fetch(`${API_URL}/api/address/${address}`);
      ...
      console.error("[MockService] getAddressInfo error", e);
  ...
  public async getAccountAddresses(
    ...
    try {
      const res = await fetch(`${API_URL}/api/account/${accountId}/addresses`);
      if (!res.ok) return [];
      const data: {
        address: string;
        address_type: number;
      ...
      console.error("[MockService] getAccountAddresses error", e);
      return [];
    }
  }
  ...
  public async connectWallets(
    ...
    console.log(
      "[MockService] connectWallets called",
  ...
  public async getLeaderboard(
    ...
    try {
      ...
      console.error("[MockService] getLeaderboard error", e);
      return {
        success: true,
        data: [],
        timestamp: Date.now(),
  ...
  public async play(
    ...
    try {
      ...
      console.log("[MockService] Play function called from UI");
      console.log("[MockService] Sending play transaction:", conciseData);
      ...
      console.error("[MockService] play error", e);
      return {
        success: false,
  ...
  /**
   * Stub – Race results are handled by the scheduled executeRace transition.
   * This is kept for backwards compatibility but doesn't need to do anything.
   */
  public async saveRaceResult(
    _result: RaceResult,
  ): Promise<NetworkResponse<boolean>> {
    console.log(
      "[MockService] saveRaceResult – race results handled by executeRace",
    );
    return {
      success: true,
      data: true,
...
async function sendMintToBatcher(
  ...
  console.log("[MockService] localWallet keys:", Object.keys(localWallet));
  if (localWallet.provider) {
    console.log(
      "[MockService] localWallet.provider keys:",
      Object.keys(localWallet.provider),
    );
    console.log(
      "[MockService] localWallet.provider constructor:",
